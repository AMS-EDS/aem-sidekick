  ...
  try {
    ...
    const addRulesPromises = projects.map(async ({
      ...
      if (authToken) {
        ...
        const corsFilters = [`^https://[0-9a-z-]+--[0-9a-z-]+--${owner}.(aem|adobems-aem|gov-aem).(page|live|reviews)/.*`];
      ...
      if (siteToken) {
        rules.push({
          ...
          condition: {
            regexFilter: `^https://[a-z0-9-]+--${repo}--${owner}.(aem|adobems-aem|gov-aem).(page|live|reviews)/.*`,
